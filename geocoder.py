...
class ColumnMapper:
    """Maps CSV columns to standardized field names using JSON configuration"""

    def __init__(self, config_file="column_mappings.json"):
    ...
    def _load_mappings(self):
        ...
        if os.path.exists(self.config_file):
            try:
                ...
                logging.warning(
                    f"Could not load column mappings from {self.config_file}: {e}"
                )
                return {}
        return {}
    ...
    def get_column_mapping(self, source_type):
        """Get column mapping for a specific source type"""
        return self.mappings.get(source_type, {}).get("columns", {})

    def get_file_pattern(self, source_type):
        """Get file pattern for a specific source type"""
        return self.mappings.get(source_type, {}).get("file_pattern", "")

    def extract_field(self, row, field_name, source_type):
        """Extract a field value from a row using the column mapping"""
        mapping = self.get_column_mapping(source_type)
    ...
    def extract_contact_data(self, row, source_type):
        """Extract all contact data from a row using the column mapping"""
        return {
            "first_name": self.extract_field(row, "first_name", source_type),
            "last_name": self.extract_field(row, "last_name", source_type),
            "email": self.extract_field(row, "email", source_type),
            "phone": self.extract_field(row, "phone", source_type),
            "status": self.extract_field(row, "status", source_type),
            "street": self.extract_field(row, "street", source_type),
            "city": self.extract_field(row, "city", source_type),
            "state": self.extract_field(row, "state", source_type),
            "zip": self.extract_field(row, "zip", source_type),
            "country": self.extract_field(row, "country", source_type),
        }
    ...
    def _load_cache(self):
        ...
        if os.path.exists(self.cache_file):
            try:
                ...
                logging.warning(
                    f"Could not load cache from {self.cache_file}, starting fresh"
                )
                return {}
        return {}
...
class EnhancedGeoMapper:
    ...
    def __init__(self, config=None):
        ...
        self.geolocator = Nominatim(
            user_agent="EnhancedGeoMapper", timeout=self.config["timeout"]
        )
        self.map = folium.Map(
        ...
        for category in categories:
            ...
            feature_group = folium.FeatureGroup(
                name=category, overlay=True, control=True
            )
    ...
    def geocode_address(self, address_dict):
        ...
        original_address = (
            ", ".join(original_address_parts)
            if original_address_parts
        ...
        if cached_result:
            ...
            if cached_result.get("failed", False):
                logging.debug(
                    f"Cache hit - previously failed address: {address_dict} (reason: {cached_result.get('failure_reason', 'Unknown')})"
                )
                ...
                display_address = (
                    original_address
                    if cached_result.get("strategy_used")
                    in ["City Only", "Full State Name", "With USA"]
                    else cached_address
                )
                return (
            ...
            for attempt in range(self.config["max_retries"]):
                try:
                    ...
                    if location:
                        ...
                        logging.info(
                            f"Successfully geocoded using {strategy_name}: {address_string} -> {full_address}"
                        )
                        return lat, lon, display_address, strategy_name
                    else:
                        logging.debug(
                            f"No results for {strategy_name}: {address_string}"
                        )
                        break

                except (GeocoderTimedOut, GeocoderUnavailable) as e:
                    logging.warning(
                        f"Geocoding attempt {attempt + 1} failed for {address_string}: {e}"
                    )
        ...
        logging.warning(
            f"All geocoding strategies failed for address: {address_dict} - cached for future reference"
        )
        self.stats["failed_geocodes"] += 1
    ...
    def _build_geocoding_strategies(self, address_dict):
        ...
        if street and (
            "po box" in street.lower()
            or "p.o. box" in street.lower()
            or street.lower().startswith("box ")
        ):
        ...
        if street and city and state:
            ...
            if enhanced_street != street:
                enhanced_address = ", ".join(
                    filter(None, [enhanced_street, city, state, postalcode])
                )
        ...
        if street and city and state:
            ...
            if street_without_number != street:
                no_number_address = ", ".join(
                    filter(None, [street_without_number, city, state, postalcode])
                )
        ...
        if street and city and state:
            ...
            if street_name_only and street_name_only != street:
                name_only_address = ", ".join(
                    filter(None, [street_name_only, city, state])
                )
        ...
        if city and state:
            ...
            if full_state != state:
                full_state_address = ", ".join(
                    filter(None, [street, city, full_state, postalcode])
                )
        ...
        if city and state:
            usa_address = ", ".join(
                filter(None, [street, city, state, postalcode, "USA"])
            )
    ...
    def _extract_street_name_only(self, street):
        ...
        cleaned = re.sub(
            r"^(north|south|east|west|n|s|e|w)\s+", "", cleaned, flags=re.IGNORECASE
        ).strip()
    ...
    def _get_offset_coordinates(self, lat, lon, person_data):
        ...
        radius = (
            0.001 + (offset_index // 8) * 0.001
        )  # Increasing radius for more people
    ...
    def create_marker(
        ...
        target_cluster = self.category_groups.get(
            category, self.category_groups["Other"]
        )
        ...
        folium.Marker(
            ...
            icon=folium.Icon(
                color=color, icon=icon, icon_color=icon_color, prefix="fa"
            ),
            tooltip=f"{first_name} {last_name} ({status})",
        ).add_to(target_cluster)

    def process_file(self, csv_file, source_type, test_limit=None):
        """Process a CSV file using column mapping from JSON configuration"""
        logging.info(f"Starting to process {source_type} from {csv_file}")
        ...
        # Get column mapping for this source type
        column_mapping = self.column_mapper.get_column_mapping(source_type)
        if not column_mapping:
            logging.error(f"No column mapping found for source type: {source_type}")
            return

        logging.info(f"Using column mapping for {source_type}: {column_mapping}")

        # Check available columns
        available_columns = df.columns.tolist()
        logging.info(f"Available columns in {csv_file}: {available_columns}")

        # Verify required columns exist (first_name, last_name, city, status)
        required_standard_fields = ["first_name", "last_name", "city", "status"]
        missing_required = []
        for field in required_standard_fields:
            csv_column = column_mapping.get(field)
            if csv_column and csv_column not in available_columns:
                missing_required.append(f"{field} (mapped to '{csv_column}')")

        if missing_required:
            logging.error(f"Missing required columns: {missing_required}")
            return

        # Filter out rows with missing critical data (city and status)
        city_column = column_mapping.get("city")
        status_column = column_mapping.get("status")

        if city_column and status_column:
            df_filtered = df.dropna(subset=[city_column, status_column])
        else:
        ...
        logging.info(
            f"Processing {file_stats['total_records']} records from {csv_file}"
        )

        for index, row in df_filtered.iterrows():
            try:
                # Extract data using column mapper
                contact_data = self.column_mapper.extract_contact_data(row, source_type)

                first_name = contact_data["first_name"] or "Unknown"
                last_name = contact_data["last_name"] or "Contact"
                email = contact_data["email"] or "None"
                phone = contact_data["phone"] or "None"
                status = contact_data["status"] or "Unknown"
                ...
                # Preprocess address
                address_dict = self.preprocessor.preprocess_address(
                    contact_data["street"],
                    contact_data["city"],
                    contact_data["state"],
                    contact_data["zip"],
                ...
                if not address_dict["city"]:
                    self.stats["skipped_records"] += 1
                    logging.debug(
                        f"Skipping record with no city: {first_name} {last_name}"
                    )
                    continue

                # Geocode the address
                lat, lon, full_address, strategy_used = self.geocode_address(
                    address_dict
                )
                ...
                if lat and lon:
                    ...
                    logging.debug(
                        f"Added marker for {first_name} {last_name} at {lat}, {lon}"
                    )
                else:
                    logging.warning(
                        f"Could not geocode address for {first_name} {last_name}: {address_dict}"
                    )
    ...
    def process_all_files(self, test_limit=None):
        """Process all configured CSV files (Contacts and Leads) into a single map"""
        source_types = self.column_mapper.get_source_types()
        logging.info(f"Processing all configured sources: {source_types}")

        for source_type in source_types:
            file_pattern = self.column_mapper.get_file_pattern(source_type)
            ...
            if not matching_files:
                logging.warning(
                    f"No files found matching pattern '{file_pattern}' for source '{source_type}'"
                )
                continue

            for csv_file in matching_files:
                file_stats = self.process_file(csv_file, source_type, test_limit)
                if file_stats:
                    self.processed_files.append(
                        {
                            "filename": csv_file,
                            "source_type": source_type,
                            "records": file_stats["total_records"],
                        }
                    )
    ...
    def print_file_summary(self):
        ...
        for file_info in self.processed_files:
            print(
                f"  {file_info['filename']:<40} ({file_info['source_type']:<10}) - {file_info['records']:>5} records"
            )
            total_records += file_info["records"]

        print("-" * 60)
        print(f"  {'TOTAL':<40} {'':<10}   {total_records:>5} records")
        print("=" * 60)
        print()
    ...
    def _create_location_summaries(self):
        ...
        for coord_key, people in self.coordinate_groups.items():
            if len(people) > 1:  # Only create summaries for multiple people
                ...
                folium.Marker(
                    ...
                    icon=folium.Icon(
                        color="darkred", icon="users", icon_color="white", prefix="fa"
                    ),
        ...
        if summary_count > 0:
            ...
            logging.info(
                f"Created {summary_count} location summary markers for clustered areas"
            )
    ...
    def log_statistics(self):
        ...
        locations_with_multiple_people = sum(
            1 for people in self.coordinate_groups.values() if len(people) > 1
        )

        logging.info(f"Unique coordinate locations: {total_unique_locations}")
        logging.info(
            f"Locations with multiple people: {locations_with_multiple_people}"
        )

        cache_hit_rate = (
            (
                self.stats["cache_hits"]
                / (self.stats["cache_hits"] + self.stats["api_calls"])
            )
            * 100
            if (self.stats["cache_hits"] + self.stats["api_calls"]) > 0
            else 0
        )
        success_rate = (
            (self.stats["successful_geocodes"] / self.stats["api_calls"]) * 100
...
def main():
    ...
    # Process all configured CSV files (Contacts and Leads)
    ...
    mapper.print_file_summary()

    print(
        "Enhanced mapping complete! Check enhanced_map.html and geocoding.log for details."
    )
