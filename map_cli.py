...
import sys
import os
from geocoder import EnhancedGeoMapper
...
def main():
    ...
    parser.add_argument(
        "--delay", type=float, help="Override API delay between calls (seconds)"
    )

    parser.add_argument(
        "--stats-only", action="store_true", help="Show cache statistics and exit"
    )
    ...
    if "map_settings" in config_data:
        ...
        mapper_config["output_file"] = map_settings.get(
    ...
    cache_file = config_data.get("file_settings", {}).get(
    ...
    if args.stats_only:
        ...
        from geocoder import GeocodeCache
    ...
    if args.csv_files:
        ...
        if (
            "file_settings" in config_data
            and "input_files" in config_data["file_settings"]
        ):
            ...
            default_file = (
                config_data.get("file_settings", {}).get("contacts_csv")
                or "Contacts_2025_05_29.csv"
            )
